...
const Cart = () => {
    ...
    const fetchData = async () => {
        ...
        const response = await fetch(SummaryApiOne.addToCartProductView.url,{
            method: SummaryApiOne.addToCartProductView.method,
            credentials: "include",
            headers: {
    ...
    const increaseQty = async (id,qty) => {
        const response = await fetch(SummaryApiOne.updateCartProduct.url,{
            method: SummaryApiOne.updateCartProduct.method,
            credentials: "include",
            headers: {
                "content-type" : "application/json"
            },
    ...
    const deleteCartProduct = async (id) => {
        const response = await fetch(SummaryApiOne.deleteCartProduct.url,{
            method: SummaryApiOne.deleteCartProduct.method,
            credentials: "include",
            headers: {
                "content-type" : "application/json"
            },
    ...
    const handlePayment = async () => {
        ...
        const response = await fetch(SummaryApiOne.payment.url, {
            method : SummaryApiOne.payment.method,
            credentials: "include",
            headers: {
                "content-type" : "application/json"
            },
  ...
  return (
    <div className='container mx-auto'>
      ...
      <div className='flex flex-col lg:flex-row gap-10 lg:justify-between p-4'>
        ...
        {
            loading ? (
                <div className='h-36 bg-slate-200 rounded border border-slate-300 animate-pulse'>
                    
                </div>
            ) : (
                <div className='h-36 bg-white'>
                    <h2 className='text-white bg-red-600 px-4 py-1'>Product Summary</h2>
                    ...
                    <button className='bg-blue-500 p-2 rounded text-white w-full' onClick={handlePayment}>Payment</button>
                </div>
            )
        }
        </div>
